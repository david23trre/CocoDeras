function initSun() {
    ...
    if (!sun || !moon || !moonMask || !stars || !fireflies) {
        return;
    }
...
const DEBUG_SPEED = 1;        // Horas del juego por segundo real
    ...
    const moonTravelStart = 17;
    const moonTravelEnd = 6;
...
if (DEBUG_CYCLE) {
    ...
    hour = realHour;
        ...
        if (isDay) {
            positionBody(sun, progressBetween(hour, sunTravelStart, sunTravelEnd));
        } else {
            positionBody(moon, progressNight(hour, moonTravelStart, moonTravelEnd));
            positionBody(moonMask, progressNight(hour, moonTravelStart, moonTravelEnd));
    ...
    function interpolateSky(from, to, progress) {
        return {
            ...
            fireflies: lerp(from.fireflies, to.fireflies, progress)
        };
    }
    ...
    function applySkyState(state) {
        ...
        document.documentElement.style.setProperty("--fireflies-opacity", state.fireflies);
    }
    ...
    function createFireflies(container) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            firefly.style.top = (60 + Math.random() * 20) + "%";
            firefly.style.setProperty("--x", (-20 + Math.random() * 40) + "px");
            firefly.style.setProperty("--y", (-10 + Math.random() * 20) + "px");
            firefly.style.setProperty("--duration",
                (5 + Math.random() * 4) + "s"
            );
            firefly.style.setProperty("--delay",
                (-Math.random() * 8) + "s"
            );
